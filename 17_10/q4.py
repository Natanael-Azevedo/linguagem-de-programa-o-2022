'''

Questão 4



Você deve escrever um programa para calcular a soma de números pares. A entrada consiste de um número inteiro positivo N. Você deve imprimir a soma dos números pares positivos menores ou iguais a N.

Para isso, você deve escrever duas funções:
    1. Uma função que recebe um número inteiro e retorna uma lista com todos os números pares no intervalo de 1 até N.
    2. Uma função que recebe uma lista e retorna a soma dos elementos da lista.


Veja os exemplos de entrada/saída abaixo.

- Exemplo de Entrada 1:
5

- Exemplo de Saída 1:
6

Os pares positivos entre 1 e 5 são 2 e 4, e a saída foi 6 porque 2 + 4 = 6.


- Exemplo de Entrada 2:
8

- Exemplo de Saída 2:
20

Neste caso, calculamos a soma de 2 + 4 + 6 + 8 = 20
'''

def listar(N):  
    for i in range(N):
        if i%2==0:
            lista.append(i)
        print(lista)
    return lista

def somar(listar):
    soma=0
    for i in listar:
        soma=soma+i
        print(soma)
    return soma
        


N = int(input())
lista=[]
